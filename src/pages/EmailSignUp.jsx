...
export default function EmailSignUp(params) {
    ...
    const error = {}
        ...
        if(!userData.password){
            error.password="please enter Phone number"
        }
  ...
  const handleNext = ()=>{
    ...
    return(
        <Container>
        <div className=" mt-10 w-1/3 px-5 py-10 flex flex-col gap-5 items-center bg-white  max-xl:w-1/2  max-md:w-2/3  max-sm:w-4/5">
            ...
            <div className=" w-full p-1 flex flex-col gap-5 items-start">
                ...
                <Input onChange={handleOnChange} name="email" placeholder="email" type="email" />
                {err.email && <p className="text-sm px-2 mt-[-10px] text-red-700 " >{err.email}</p>}
                <Input onChange={handleOnChange} name="password" placeholder="Password"  type="password" />
                {err.password && <p className="text-sm px-2 mt-[-10px] text-red-700 " >{err.password}</p>}
             <Link to="/phone-signup"> <p className="px-2 leading-tight tex-lg text-sm text-[#1DA1F2]  ">Use Phone number</p></Link>  
            </div>
